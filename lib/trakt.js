  ...
  return new Promise(function(resolve, reject) {
    request("shows/" + slug + "/seasons/" + season + "/?extended=full,images", (err, res, body) => {
      if (err && retry) {
        ...
        reject(err + " with link: 'shows/" + (config.colorOutput ? colors.cyan(slug) : slug) + "/seasons/" + (config.colorOutput ? colors.cyan(season) : season) + "/?extended=full,images'");
      } else if (!body || res.statusCode >= 400) {
  ...
  return new Promise(function(resolve, reject) {
    request("shows/" + slug + "/watching", (err, res, body) => {
      if (err && retry) {
        ...
        reject("Trakt: Could not find watching info on: '" + (config.colorOutput ? colors.cyan(slug) : slug) + "'");
      } else {
        resolve(JSON.parse(body));
      }
    });
