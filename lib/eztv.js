...
module.exports = {
  ...
  getShowDetails: (data) => {
    ...
    request(SHOWS + data.id + "/" + data.slug + "/", (err, res, body) => {
      if (err) {
        ...
        defer.reject("EZTV: Could not find imdb for: '" + (config.colorOutput ? colors.cyan(data.slug) : data.slug) + "'");
      } else {
        const $ = cheerio.load(body);
  ...
  getAllEpisodes: (data) => {
    ...
    request(SHOWS + data.id + "/" + data.slug + "/", (err, res, body) => {
      if (err) {
        defer.reject(err + " with link: '" + SHOWS + data.id + "/" + (config.colorOutput ? colors.cyan(data.slug) : data.slug) + "/'");
      } else if (!body || res.statusCode >= 400) {
        defer.reject("EZTV: Could not find episodes for: '" + (config.colorOutput ? colors.cyan(data.slug) : data.slug) + "'");
      } else {
        const $ = cheerio.load(body);
